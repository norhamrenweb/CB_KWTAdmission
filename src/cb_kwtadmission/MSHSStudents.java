...
public class MSHSStudents {
       ...
       public static void getStudents() throws SQLException {
        ...
        Class.forName("com.microsoft.sqlserver.jdbc.SQLServerDriver");
            ...
            ResultSet rs = st.executeQuery("Select td.id,td.note as assesdate,si.studentid as studentid from trackingdata td inner join studentinquiry si on td.id=si.inquiryid where td.trackingsystemid = 4 and itemnumber = 2");
            while (rs.next()) {
            ...
            if(studentids.isEmpty()){
                File f = new File(hoy+"noStudentsFoundMSHS.log");
            ...
            for (int i = 1; i <studentids.size();i++) {
                ...
                if (rs1.next()) {
                    ...
                    File f = new File(hoy+"cellphonefoundMSHS.log");
                    FileWriter fichero = null;
                    try {
...
"تذكير: غدا هو يوم الاختبار لإبنك/ ابنتك  في الساعة  "+hour+" في المدرسة الكندية ثنائية اللغة"   ;
                    ...
                    File f = new File(hoy+"cellphonenotfoundMSHS.log");
                    FileWriter fichero = null;
                    try {
                        fichero = new FileWriter(f.getAbsoluteFile(),true);
